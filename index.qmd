...
#               color = "black", fill = "lightpink")+
  ...
  labs(x = "", 
      ...
      subtitle = "Faceted by Selected Genres",
       fill="Count",
       caption = "Source: Wikipedia.org") +theme_void()
...
state_stations_pop_reg <- distinct(state_stations_pop %>% group_by(state) %>% 
                           summarise(n = n(), region_name = region_name, pop_2022 = POPESTIMATE2022))
```
...
ggplot(state_stations_pop_reg, aes(x = pop_2022, y = n, color = region_name)) +
  ...
  geom_text_repel(data=subset(state_stations_pop_reg, pop_2022 > 15000000 | n > 500), aes(label = state), color = "black", size = 3) +
  scale_x_continuous(labels = label_number(scale_cut = cut_short_scale())) +
  scale_color_manual(values = c("#333333", "#E69F00", "#56B4E9", "#009E73")) +
  labs(x = "Population", y = "Number of Radio Stations", title = "Population vs. Number of Radio Stations in the US",
       subtitle = "Colored by Region",
       color = "Region") + theme_minimal()
```
...
trial<-station_count%>%
  mutate(newpopulation=population/10000)%>%
  pivot_longer(cols=c("n", "newpopulation"),
               names_to="type",
               values_to='amount')
ggplot(data=trial, aes(x=amount, fill= type)) +
 geom_histogram(alpha = 0.3,bins=45,position="identity") + xlim(0,1500)+
  theme_minimal()+
    labs(
