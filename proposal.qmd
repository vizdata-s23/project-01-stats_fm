...
The data collected from all 50 states were then aggregated to give the final data this project is using, thus adding another variable, state.
...
Another question we are interested in answering is the effect of population size on the number of radio stations. Given the variation of population density across the US, we would like to identify any differences in this relationship based on region (West, Midwest, South, Northeast).
...
To find a relationship between population of a state and the number of radio stations we will plot a scatter plot where each data point is a different state. The x-axis will represent the state's population and the y-axis will represent the number of radio stations in the state. To accomplish this, we will need to import another data set that contains the population of each state. The variables we plan to use are state, call sign, region (to be created by us) and state population. As stated before, we would also like to check for any differences in this relationship based on regions in the US (Regions are defined here: <https://vividmaps.com/regions-united-states/>), which will be separated by color.
